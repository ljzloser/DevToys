...
{
	//connect(&monitor_timer__, &QTimer::timeout, this, &SystemInfo::GetResource);
	//monitor_timer__.start(m_timer_interval__);
}
void SystemInfo::GetResource()
{
	double nCpuRate = 0;
...
bool SystemInfo::GetMemUsage(double& nMemTotal, double& nMemUsed)
{
	...
	emit this->memRateChanged(nMemUsed / nMemTotal * 100);
	return true;
#endif
}
bool SystemInfo::GetNetUsage()
{
#if defined(Q_OS_LINUX)
	QProcess process;
...
bool SystemInfo::GetCpuUsage(double& nCpuRate)
{
	nCpuRate = -1;
#if defined(Q_OS_LINUX)
	QProcess process;
	...
	double CpuRate = 100.0 * (double)(kernel + user - idle) / (double)(kernel + user);

	//static FILETIME preIdleTime = { 0, 0 };
	//static FILETIME preKernelTime = { 0, 0 };
	//static FILETIME preUserTime = { 0, 0 };
	//FILETIME idleTime, kernelTime, userTime; // 这几个变量代表意思是
	//GetSystemTimes(&idleTime, &kernelTime, &userTime);
	//long long idleDiff = FileTimeToInt64(idleTime) - FileTimeToInt64(preIdleTime);
	//long long kernelDiff = FileTimeToInt64(kernelTime) - FileTimeToInt64(preKernelTime);
	//long long userDiff = FileTimeToInt64(userTime) - FileTimeToInt64(preUserTime);

	//long long systemDiff = kernelDiff + userDiff;
	//long long totalDiff = systemDiff - idleDiff;

	//preIdleTime = idleTime;
	//preKernelTime = kernelTime;
	//preUserTime = userTime;
	//nCpuRate = totalDiff > 0 && systemDiff > 0 ? (double(totalDiff) / double(systemDiff)) * 100.0 : 0.0;
	emit this->cpuRateChanged(CpuRate);
	//qDebug() << "windows:CPU use rate:" << nCpuRate << "%";
#endif
	return true;
}
bool SystemInfo::GetDiskSpeed()
{
#if defined(Q_OS_LINUX)
	QProcess process;
...
bool SystemInfo::GetdiskSpace(unsigned long& lFreeAll, unsigned long& lTotalAll)
{
#if defined(Q_OS_LINUX)
	QProcess process;
	process.start("df -k");
	...
	//qDebug() << lTotalAll - lFreeAll << (lTotalAll - lFreeAll) / lTotalAll << ((lTotalAll - lFreeAll) / lTotalAll) * 100;
	emit this->diskRateChanged(((lTotalAll - lFreeAll) / (double)lTotalAll) * 100);
#endif
	return true;
}
bool SystemInfo::GetPathSpace(const QString& path)
{
#if defined(Q_OS_LINUX)
