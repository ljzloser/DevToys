	...
	connect(ui.importPlainTextEdit, &QPlainTextEdit::textChanged, [=]()
		{
			...
			{
				ui.exportsPlainTextEdit->setPlainText(Tools::urlEncode(text));
			}
			else
			{
				ui.exportsPlainTextEdit->setPlainText(Tools::urlDecode(text));
			}
		});
	connect(ui.slideButton, &SlideButton::clicked, [=]()
		{
			if (!ui.slideButton->isChecked())
			{
				ui.importPlainTextEdit->setPlainText(Tools::urlEncode(ui.importPlainTextEdit->toPlainText()));
			}
			else
			{
				ui.importPlainTextEdit->setPlainText(Tools::urlDecode(ui.importPlainTextEdit->toPlainText()));
			}
		});
}
